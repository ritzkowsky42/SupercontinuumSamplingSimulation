...
pot1 = PotentialParams(E_fermi = E_fermi, a = a, cB = cB, Wf = Wf, Bias_field = Bias_field, d_abl = d_abl, V_max = V_max,Type="step", ROC = 15/x0, FE = 10)
...
CSV.write("SampledWaveform10Vnm.csv", DataFrame(time = delayVals*t0, field = qAvg./maximum(qAvg)), writeheader = true)
