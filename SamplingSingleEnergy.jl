...
for (i,val) in enumerate(pulseList)
    push!(fieldList,addPulse(pumpPulseField, val))
end
...
ax2.plot(delayVals*t0,samplingCurrent)
ax2.plot(delayVals*t0,refpulse.E)
ax2.plot(delayVals*t0,analyticalCurrent)
